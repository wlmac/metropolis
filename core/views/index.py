...
class Index(TemplateView, mixins.TitleMixin):
	...
	def get_context_data(self, **kwargs):
		...
		events1 = (
			lambda: models.Event.get_events(user=self.request.user).filter(end_date__gte=datetime_now, ).order_by(
				"start_date"))
		events = list(events1().filter(~Q(schedule_format="default"), )[:1])
		events += events1().filter(Q(schedule_format="default"), )[: 3 - len(events)]
...
class CalendarFeed(ICalFeed, View):
	...
	def items(self):
		...
		return (models.Event.get_events(user=None).filter(end_date__gte=now - padding,
			# todo add ?start= and ?end= to url via self.request.query_params
			start_date__lte=now + padding, ).order_by("-start_date"))
	...
	def item_categories(self, item):
		return ([tag.name for tag in item.tags.all()] + (["public"] if item.is_public else []) + (
...
TeamData = Dict[str,  # Team role (e.g., "Project Manager", "Frontend Developer", etc.)
...
class Justinian(View):
	def get(self, request):
		return HttpResponse(
			"orz our exalted leader justinian luxtel, waba skyflarus, as payment for his services<br>" + gs.justinian_slam(),
			status=402, )
