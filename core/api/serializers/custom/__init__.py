...
class PrimaryKeyAndSlugRelatedField(serializers.SlugRelatedField):
	...
	def to_representation(self, obj):
		if self.pk_field is not None:
			...
			pk = obj.pk
		return {"id": pk, "slug": getattr(obj, self.slug_field), }
...
class ContentTypeField(serializers.ChoiceField):
	def __init__(self, **kwargs):
		...
		choices = CI_safe(
			ContentType.objects.filter(app_label="core", model__in=settings.POST_CONTENT_TYPES).values_list("model",
			                                                                                                "model"))
		default_error_messages = {"does_not_exist": "ContentType with model '{value}' does not exist."}
...
class CommentSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = Comment
		fields = ["id", "body", "author", "has_children", "created_at", "edited", "likes", ]
...
class SingleUserField(serializers.Field):
	...
	def __init__(self, **kwargs):
		default_error_messages = {"does_not_exist": "User with ID {value} does not exist.", }
	...
	def to_internal_value(self, data):
		...
		if len(users) != len(data):
			...
			for missing_id in missing_ids:
				self.fail("invalid", message=f"User with ID {missing_id} does not exist.")
...
class OrganizationField(ChoiceField):
	def __init__(self, **kwargs):
		self.slug_field = "model"
		choices = CI_safe(Organization.objects.filter(is_active=True).values_list("id", "name"))
...
class UserOrganizationField(OrganizationField):
	...
	def to_internal_value(self, data):
		...
		if len(organizations) != len(data):
			...
			for missing_id in missing_ids:
				self.fail("invalid", message=f"Organization with ID {missing_id} does not exist.", )
...
class TagRelatedField(MultipleChoiceField):
	...
	def to_internal_value(self, data):
		...
		if len(tags) != len(data):
			...
			for missing_id in missing_ids:
				self.fail("invalid", message=f"Tag with ID {missing_id} does not exist.")
