...
def notif_broker_announcement(obj_id):
    ...
    if ann.organization.id in settings.ANNOUNCEMENTS_NOTIFY_FEEDS:
        ...
        affected = affected.filter(Q(tags_following__in=ann.tags.all()) | Q(organizations__in=[ann.organization]))
...
def notif_events_singleday(date: dt.date=None):
    ...
    for u in eligible.all():
        ...
        covering = list(Event.get_events(u).filter(
        ...
        headline = covering[0]
        notif_single.delay(u.id, dict(
            title=_l("%(date)s: %(headline)s") % dict(date=date.strftime("%a %b %d"), headline=headline.name),
            body=body,
