...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--dry-run', action='store_true', help="Just show what courses will be updated or skipped; don't actually write them.")
        ...
        try:
            with open(options['json_file'], 'r') as f:
                ...
                for course_json in dirty_data:
                    ...
                    if not isinstance(course_json['code'], str) or not isinstance(course_json['position'], int):
                        raise InvalidCourseJSONFileError

                    data.append({'code': course_json['code'], 'position': course_json['position']})
        ...
        for course_json in data:
            ...
            try:
                ...
                if course.position == course_position:
                    ...
                    if not dry_run: course.save()

                    num_courses_updated += 1

                    self.stdout.write(self.style.NOTICE(f'Updated course {course_code} from position {course_previous_position} to {course_position}'))
            except models.Course.DoesNotExist:
                if not dry_run: models.Course.objects.create(term=term, code=course_code, position=course_position)

                num_courses_added += 1

        self.stdout.write(self.style.SUCCESS(f'{num_courses_added} courses added, {num_courses_updated} updated, {num_courses_unmodified} unmodified.'))
